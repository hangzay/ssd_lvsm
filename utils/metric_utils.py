...
def summarize_evaluation(evaluation_folder, avg_sample_latency=None):
    ...
    # export average metrics to a text file; append latency if provided
    lines = [f"Average: {','.join(averages)}\n"]
    if avg_sample_latency is not None:
        lines.append(f"avg_sample_latency: {avg_sample_latency:.4f}s\n")
