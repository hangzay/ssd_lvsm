...
class ProcessData(nn.Module):
    ...
    def forward(self, data_batch, has_target_image=True, target_has_input=True, compute_rays=True):
        ...
        if compute_rays:
            for view_dict in [input_dict, target_dict]:
                c2w = view_dict["c2w"]
                fxfycxcy = view_dict["fxfycxcy"]
                image_height, image_width = view_dict["image_h_w"]

                ray_o, ray_d = self.compute_rays(c2w, fxfycxcy, image_height, image_width, device=data_batch["image"].device)
                view_dict["ray_o"], view_dict["ray_d"] = ray_o, ray_d

        return input_dict, target_dict


def decoupled_nvs_collate_fn(batch):
    out = {}
    keys = batch[0].keys()

    if "scene_name" in keys:
        out["scene_name"] = [sample["scene_name"] for sample in batch]
